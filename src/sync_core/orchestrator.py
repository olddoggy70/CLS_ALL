"""
Main coordination logic for parquet update operations
"""
...
from .processing import (
    apply_categorical_types,  # Runtime categorical casting
    apply_incremental_update,  # Unified function for single AND batch processing
    process_weekly_full_backup,  # Weekly full mode
    rebuild_parquet,  # Fallback mode
)
from .quality import (
    print_change_summary,  # Fallback mode
    track_row_changes,  # Fallback mode
)
...
def _clean_change_summary_for_state(change_summary: dict | None) -> dict | None:
    """
    Remove non-JSON-serializable objects (like DataFrames) from change_summary before saving to state

    Args:
        change_summary: Original change summary dict

    Returns:
        Cleaned dict suitable for JSON serialization
    """
